...
InverterVFD::InverterVFD(HardwareSerial &serial, int dePin, unsigned long baud)
  : modbusSerial(serial), pinDE(dePin), currentBaud(baud), slaveAddr(1), debug(false) {}
...
void InverterVFD::txEnable() {
  digitalWrite(pinDE, HIGH);
  delayMicroseconds(100);
}

void InverterVFD::txDisable() {
...
bool InverterVFD::readRegister(uint16_t reg, uint16_t &outValue, uint16_t timeout_ms) {
  ...
  if (!sendRequestAndReceive(req, sizeof(req), resp, respLen, timeout_ms)) return false;
  if (respLen < 7 || resp[0] != slaveAddr || (resp[1] & 0x80)) return false;
...
bool InverterVFD::start() {
  return writeRegister(REG_RUN_COMMAND, 0x0001);
}

bool InverterVFD::stop() {
  return writeRegister(REG_RUN_COMMAND, 0x0000);
}

bool InverterVFD::readOutputFrequency(float &hz) {
  uint16_t raw;
  if (!readRegister(REG_OUTPUT_FREQ, raw)) return false;
  hz = raw * SCALE_FREQ;
  return true;
}
...
bool InverterVFD::readOutputCurrent(float &ampere) {
  uint16_t raw;
  if (!readRegister(REG_OUTPUT_CURR, raw)) return false;
  ampere = raw * 0.01f;
  return true;
}
